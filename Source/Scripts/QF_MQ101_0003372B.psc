...
;BEGIN CODE
; Since we are still sitting on the Helgen Inn Balcony we dont really need any Music Action. The player is
; just sitting on there. Id argue its probably more disruptive than anything, so lets remove it
...
;END AUTOCAST
;BEGIN CODE
; Intro officially starts here from APs POV, removing all Actors n Stuff that are relevant to pre-Intro
GameHour.Value += 3 ;game time
If(GameHour.Value > 23)
  GameHour.Value -= 24
EndIf

; Helgen but would disrupt the Intro or Post-Intro Helgen
...
; Also kick the Fort Soldier out of the keep, otherwise they take too long to get to their location
Alias_ImperialSoldierFort01.GetReference().MoveTomyEditorLocation()

; and the Archers should move to their position now too
Alias_HelgenArcher01.GetActorRef().EvaluatePackage()
Alias_HelgenArcher02.GetActorRef().EvaluatePackage()
;END CODE
EndFunction
;END FRAGMENT
...
;BEGIN FRAGMENT Fragment_376
Function Fragment_376()
;BEGIN CODE
...
;Alias_Hadvar.TryToMoveTo(HadvarKeepMarker1)
;Alias_Hadvar.GetReference().Enable()
ObjectReference Ralof = Alias_Ralof.GetReference()
Ralof.Enable()
Ralof.MoveTo(RalofKeepMarker1)
(Ralof as Actor).RemoveItem(PrisonerCuffs)
...
; boith gates closed now, disable Collision Markers again
...
;BEGIN FRAGMENT Fragment_378
Function Fragment_378()
;BEGIN CODE
APMQ101PreIntroScene.Start()
;END CODE
EndFunction
;END FRAGMENT
...
; We still set GameHour to 7 to let the Player start at a new day. Well do that again when the
; Intro Scene actually starts \o/
GameHour.SetValue(7)
...
;BEGIN CODE
; it is kind of sad to do really anything here but there is one tinsy tiny line changing the Players
; view direction.. we obviously dont want that
...
;BEGIN FRAGMENT Fragment_375
Function Fragment_375()
;BEGIN CODE
APMain.DestroyHelgen()
;END CODE
EndFunction
...
;	debug.trace(self + " moving Hadvar to " + HelgenFriendMarker)
	...
	Hadvar.enable()
	Hadvar.moveto(HelgenFriendMarker)
	; start player on Imperial side
	kmyQuest.FactionPath = 1
	MQ102A.setstage(1)
else
...
; 	debug.trace(self + " moving Ralof to " + HelgenFriendMarker)
	ObjectReference Ralof = Alias_Ralof.GetReference()
	Ralof.Enable()
	Ralof.moveto(HelgenFriendMarker)
	; start player on Sons side
	kmyQuest.FactionPath = 2
	MQ102B.setstage(1)
endif
...
SkyrimClear.ForceActive(true) ; weather override
; GameHour.SetValue(7) ;game time < setting in Stage5
MUSDungeonChargen.Add() ; Play music \o/ idk if this is necessary, lets just try it

; We still want to disable part of the Players controls however to avoid them starting a fight now.. or doing anything stupid
...
; starting intro scene, vanilla game doesnt need this as it starts this Scene on Quest start
; but we delayed the Scene start, doing it here now instead
Scene1.Start()

Debug.Trace("AP: Setting Stage 12")
SetStage(12)
;END CODE
EndFunction
;END FRAGMENT
...
;BEGIN CODE
;/ Do nothing. Vanilla blends in the lil help info of looking around with mouse but
I dont expect this mod to be used for a 1st time player so whats the point. You should now the
controls
AP doesnt even call this stage, this is just for documentation
/;
;END CODE
EndFunction
;END FRAGMENT
...
;BEGIN CODE
; Imperials, inside keep
; move everybody inside keep
Alias_TrophyRoomPrisoner01.TryToEnable()
Alias_TrophyRoomPrisoner02.TryToEnable()
ObjectReference Hadvar = Alias_Hadvar.GetReference()
Hadvar.Enable()
Hadvar.MoveTo(HadvarKeepMarker1)
...
;END AUTOCAST
;BEGIN CODE
; Removing all Music to build up some.. uh.. tension
...
ObjectReference Property MiscNPCMarker  Auto
...
Scene Property APMQ101PreIntroScene  Auto  
